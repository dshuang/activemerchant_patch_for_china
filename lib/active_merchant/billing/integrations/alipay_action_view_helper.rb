  ...
  module Billing #:nodoc:
    module Integrations #:nodoc:
      module AlipayActionViewHelper
        ...
        #  <% end %>
        #
        def payment_service_for_alipay(order, account, options = {}, &proc)          
          raise ArgumentError, "Missing block" unless block_given?

          integration_module = ActiveMerchant::Billing::Integrations.const_get(options.delete(:service).to_s.camelize)

          result = []
          result << form_tag(integration_module.service_url, options.delete(:html) || {}).gsub(/<div.*/,'') # gets rid of the auto-generated utf8 parameter which interferes with alipay processing the request
